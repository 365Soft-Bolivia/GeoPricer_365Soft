...
const saveChanges = async () => {
  ...
  try {
    if (editSection.value === 'basica') {
      await router.put(proyectos.update(props.producto.id).url, editForm.value);
...
const toggleConfig = async (field: string) => {
  ...
  try {
    await router.put(proyectos.update(props.producto.id).url, {
      [field]: newValue
    });
  } catch (error) {
          ...
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex items-center gap-3">
              <span
                ...
                class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold cursor-pointer hover:opacity-80"
                style="font-family: 'Montserrat', sans-serif;"
                @click="toggleConfig('estado')"
              >
                <span class="h-2 w-2 rounded-full" :style="producto.estado === 1 ? 'background: #10b981;' : 'background: #E0081D;'"></span>
